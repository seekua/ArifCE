    ...
    public static async Task<int> RunAsync(string[] args)
    {
        try
        {
            ...
            switch (command)
            {
                ...
                case "verify": Require(args, 2, "verify <claim-id> --command <command> [--path <file-or-directory>] [--allow-unsafe-command]"); var verified = await service.VerifyAsync(root, args[1], Option(args, "--command") ?? throw new ArgumentException("--command is required."), args.Contains("--allow-unsafe-command", StringComparer.OrdinalIgnoreCase), Options(args, "--path")); Console.WriteLine($"{verified.Claim.Id}: {verified.Claim.Status} ({verified.Evidence.Id})"); break;
