...
The post-V0.7 LLM objective is in progress: local provider adapters (OpenAI, Anthropic, Gemini, OpenRouter, Ollama, LM Studio), local API-key profiles, fallback/task routing, connection tests, token/cost accounting, canonical LLM evidence, dashboard LLM activity, approval policy primitives, benchmark primitives, and a deterministic hash-vector test stub are implemented. Semantic embeddings are not implemented. Remaining work is to wire richer policy/reviewer workflows, IDE manifest capabilities, expanded MCP tools, multi-project orchestration, and full end-to-end CI/release evidence.
