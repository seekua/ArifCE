...
foreach ($evaluator in $evaluatorRows) {
    ...
    if ($evaluator.fixture -notin @('behavior','llm','mcp','safety','storage','freshness','propagation','graph','contract')) { throw "Evaluator $($evaluator.taskId) has an unknown fixture." }
