[CmdletBinding()]
param([ValidateSet('trust-dirty-content','llm-secret-boundary','acceptance-risk-policy','canonical-concurrency','stale-propagation','deterministic-code-graph','change-impact-contract','structured-flight-recorder')][string]$TaskId = 'trust-dirty-content')
...
try {
    ...
    if ($registry.evaluators[0].fixture -notin @('safety','storage','freshness','propagation','graph','contract','flight-recorder')) { $registry.evaluators[0].sourceCommit = $manifest.fixtureCommit }
