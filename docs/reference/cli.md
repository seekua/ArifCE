...
`context --task` pins the canonical task contract and current completion state, then deterministically considers records explicitly linked to that task: failed attempts, claims and their evidence, unresolved findings, and the latest task handoff. Lexical retrieval fills only the remaining estimated-token budget. Records explicitly linked to another task are reported as `OUT_OF_SCOPE` when encountered and their snippets are not serialized. When even the contract cannot fit, no partial contract is emitted. Token counts are estimates, not tokenizer-exact guarantees.
