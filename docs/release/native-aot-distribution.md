...
An npm launcher is now available at `npm/arifce`. It delegates to the locally installed .NET tool and does not imply NativeAOT support. A self-contained (non-AOT) packaging probe is available through `scripts/publish-self-contained.ps1`; it removes the .NET runtime requirement while compatibility work for NativeAOT continues.
