...
Each acceptance retains its original evidence IDs. Refresh checks that all of those records still belong to the accepted claim, remain linked to it, and are current. New sibling evidence can support a claim but cannot renew an earlier approval. Missing, malformed, foreign or unknown-freshness evidence, or a missing/non-eligible claim, moves an accepted record to `NEEDS_REVIEW`. Warnings for that state repeat in subsequent handoffs; revoked records remain revoked. Creating a new acceptance also rejects foreign evidence and disputed claims.

These lifecycle checks do not authenticate the person or process that issued evidence. The propagation regression fixtures use synthetic low-risk evidence, not actual build/review executions, and do not establish concurrent transaction isolation, policy re-evaluation after configuration changes, or complete decision/invariant graph propagation.
