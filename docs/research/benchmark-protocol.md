...
Suite preparation and collection are separate. The repeated protocol prepares forty isolated directories (20 matched pairs) but never invokes a model. Collection rejects partial or unmatched suites and verifies provenance plus independent-evaluator artifacts again before aggregating results. Execution status is recorded in `docs/evidence/engineering-benchmark-readiness.md`; the first complete run is reported in `docs/evidence/engineering-benchmark-results-2026-09-02.md`.

For repeated product studies, generate `execution-plan.json` once after preparation with `new-engineering-benchmark-execution-plan.ps1`. Its SHA-256-seeded pair order is reproducible, both arms of a pair stay adjacent, and first-arm assignment is balanced. The plan hashes each immutable session and prompt, refuses overwrite, and stores no seed plaintext, command arguments, environment variables, or absolute local paths. Follow its sequence without outcome-driven reordering.

The declared token budget is enforced as a measured post-run ceiling because the Codex host does not expose a reliable whole-agent-run token cutoff. A run that exceeds the ceiling is preserved rather than retried or omitted: its independent evaluator result remains visible, `tokenBudgetCompliant` is false, and it cannot count as a protocol pass. This prevents a correct result bought with excess context or reasoning from being silently scored as budget-matched evidence.
