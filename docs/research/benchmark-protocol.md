...
Suite preparation and collection are separate. Preparation creates all twenty isolated directories but never invokes a model. Collection rejects partial or unmatched suites and verifies provenance plus independent-evaluator artifacts again before aggregating results. Environment readiness and the absence of a real run are recorded in `docs/evidence/engineering-benchmark-readiness.md`.
