...
Log review showed that the agent started its evidence-producing `arifce verify` test while an earlier wrapper-owned test was still running. The overlapping testhosts then rebuilt and held the same assemblies; the old CLI verification process never returned. The harness now serializes bounded checks and evidence verification through one checkout-scoped named mutex, disables MSBuild node reuse, and makes the evidence-producing test use `--no-build --no-restore`. The product process runner now drains stdout and stderr concurrently and kills the whole child tree on cancellation rather than permitting a full pipe or cancellation to strand a verifier. Behavioral lock, trial-isolation, workflow-classification, completion-provenance, and large-stderr verification regressions pass locally; the full repository suite passes 120/120.

A future pair must remain predeclared and must not reuse either candidate from this run. The process-isolation remediation passed all Windows, Ubuntu, and macOS jobs, all five self-contained binary smokes, package smoke, and the Linux-only benchmark/evaluator matrix in [CI run 35701316103](https://github.com/seekua/ArifCE/actions/runs/35701316103). A fresh clean-window matched pair remains the next gate. Lower-model trials remain blocked until one matched pair passes every gate in both arms.
