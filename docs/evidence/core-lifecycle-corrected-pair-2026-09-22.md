...
A future pair must remain predeclared and must not reuse either candidate from this run. Remote cross-platform CI and a fresh clean-window matched pair remain the next gates. Lower-model trials remain blocked until one matched pair passes every gate in both arms.
