...
- **Self-contained distribution:** the repeatable CLI packaging script, dependency policy, cross-platform smoke matrix, and downloadable CI artifacts are now present. Publishing and signing every platform binary, package-manager manifests, and NativeAOT compatibility remain deferred until release signing and the recorded AOT diagnostics are resolved.
