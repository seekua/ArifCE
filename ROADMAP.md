...
- [ ] Phase 74: strengthen change-contract linkage, risk/confidence, rejection-side-effect and scoped evidence/acceptance assertions with four pinned tests and good/six-mutant controls. Existing product code passes 105 local tests; calibration, independent integration and remote closure are pending. See docs/evidence/benchmark-contract-calibration-2026-09-03.md.

## Explicit deferrals

- **Remaining evaluator calibration:** Phases 69–73 close six of ten replacements with finite calibration: secret boundary, acceptance policy, cross-process storage/index reconstruction, repository freshness, acceptance/handoff propagation and deterministic graph. Four other evaluator gaps and fresh real A/B runs remain open. Password/High-risk controls do not establish every secret format, Critical-risk policy, reviewer identity independence or evidence issuer authenticity. Storage does not establish crash recovery, network-filesystem locking, concurrent rebuilds or SQL schema migrations. Freshness does not exhaust merge/rebase/shallow clone, ignored-file, non-UTF8 filename or concurrent-capture behavior. Propagation does not establish complete decision/invariant transitivity, concurrent transaction isolation or policy-change re-evaluation. Graph tests do not establish compiler-bound semantic resolution, full MSBuild interpretation, hostile valid-JSON cache handling or concurrent capture; these controls do not establish model effectiveness.
- **Evaluator coverage remediation (FINDING-0005):** public contracts and TRX scoring are verified; six evaluators are strengthened/calibrated. Product-effectiveness claims remain disabled while change contracts, flight recorder, MCP validation and unfinished-verification policy remain. Next strengthen change-contract linkage, confidence, persisted lifecycle and trusted-scope assertions with good/bad controls. See docs/evidence/benchmark-graph-calibration-2026-09-03.md. Exposing compatibility requirements does not make partial assertions complete.
