...
- [ ] Phase 79: make the external model study fair before resuming metered execution, then execute and publish all 20 matched pairs without favorable-result filtering. The balanced order gate passed [CI run 34295202034](https://github.com/seekua/ArifCE/actions/runs/34295202034); the hardened harness passed three-OS/five-package CI in [run 34462957826](https://github.com/seekua/ArifCE/actions/runs/34462957826), and the corrected graph contract passed in [run 34466811725](https://github.com/seekua/ArifCE/actions/runs/34466811725). Three Terra/medium pairs remain diagnostic and ineligible: the first exposed an underspecified public probe; the second reached behavioral evaluation but retained a network-confounded repository check; the third passed repository/API gates in both arms but both candidates failed the same two behavior tests, while the ArifCE arm also failed to complete its treatment workflow. Benchmark-only checks disable network-dependent NuGet auditing. Retry-loop policy now requires the same command and meaningful failure, and treatment validation requires successful, separately observable rebuild/context/search/task/claim/verify/handoff operations. A fresh matched pair after remote CI and a clean usage-window reset remains pending. The 50,000 non-cached-input-plus-output target is diagnostic until passing runs calibrate it. See docs/evidence/benchmark-harness-reliability-2026-09-10.md.
