...
- **Phase 67 host timing:** process capture, integrity, completion integration, suite checks, and all 83 behavior tests pass locally; remote validation is pending. The monotonic process stopwatch excludes outer preparation/evaluation delay, but includes in-process network and approval waits. Active-work timing stays unavailable until these intervals are observable. Interrupted captures retain artifacts but require explicit failure reporting and a separately identified retry, not silent omission. The real matched rerun remains open.
