...
- [ ] Phase 78: make the real model study repeatable and permission-matched: prepare two fresh sessions for each of ten task categories, require the same non-secret permission profile per pair, and reject product-study collection without captured token and host-time telemetry. External model execution and effectiveness claims remain out of scope until this harness passes CI and the metered trials run.
