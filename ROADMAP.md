...
- [ ] Phase 71: fix nested-untracked/literal-path/deleted-file snapshot defects and replace legacy dirty-content scoring; additionally fix the stale ID-scan data loss exposed by later Phase 70 CI (29/30 tasks). Freshness checks pass locally; ID-race calibration and remote closure pending. See docs/evidence/benchmark-freshness-calibration-2026-09-03.md.
...
- **Evaluator coverage remediation (FINDING-0005):** public contracts and TRX scoring are verified; three evaluators are now strengthened/calibrated. Product-effectiveness claims remain disabled until the remaining seven objectives are strengthened/calibrated; next is repository freshness, removing unrelated legacy shell prerequisites and testing current/changed/failure paths. See docs/evidence/benchmark-storage-calibration-2026-09-03.md. Exposing compatibility requirements does not make partial assertions complete.
