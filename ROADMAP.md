...
- **Phase 68 evaluator fairness:** all ten public contracts and coverage limits are now explicit; TRX-based scoring separates evaluator errors from executed assertions. Local contract, isolation, completion, registry/suite checks and all 83 behavior tests pass; remote CI is pending. Product-effectiveness claims are disabled for these diagnostic scores. Before a comparative rerun, replace false-positive-prone secret/acceptance checks, add actual multi-process/index coverage, and calibrate all evaluators with correct/incorrect controls; see docs/evidence/benchmark-contract-audit-2026-09-03.md. Exposing compatibility requirements does not make partial assertions complete.
