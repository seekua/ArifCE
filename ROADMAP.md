...
- [ ] Phase 79: make the external model study fair before resuming metered execution, then execute and publish all 20 matched pairs without favorable-result filtering. The balanced order gate passed [CI run 34295202034](https://github.com/seekua/ArifCE/actions/runs/34295202034). Public machine-readable API contracts, a pre-evaluator compile gate, Windows temp-root isolation, bounded check output, churn diagnostics, and a mandatory real ArifCE workflow are implemented locally; remote CI and one clean matched pilot pair remain pending. The 50,000 non-cached-input-plus-output target is diagnostic until passing runs calibrate it. See docs/evidence/benchmark-harness-reliability-2026-09-10.md.
