...
- [ ] Phase 79: make the external model study fair before resuming metered execution, then execute and publish all 20 matched pairs without favorable-result filtering. The balanced order gate passed [CI run 34295202034](https://github.com/seekua/ArifCE/actions/runs/34295202034); the hardened harness passed three-OS/five-package CI in [run 34462957826](https://github.com/seekua/ArifCE/actions/runs/34462957826). The first clean-window Terra/medium pair remains an invalid failed run: both arms passed repository tests and the then-bound public gate, but the independent evaluator exposed omitted graph-record API members. The corrected public probe now exercises that surface and rejects the legacy underspecified API. A fresh contract-bound matched pair remains pending. The 50,000 non-cached-input-plus-output target is diagnostic until passing runs calibrate it. See docs/evidence/benchmark-harness-reliability-2026-09-10.md.
