...
- **Remaining evaluator calibration:** Phases 69–71 close four of ten replacements with finite calibration: secret boundary, acceptance policy, cross-process storage/index reconstruction and repository freshness. Six other evaluator gaps and fresh real A/B runs remain open. Password/High-risk controls do not establish every secret format, Critical-risk policy, reviewer identity independence or evidence issuer authenticity. Storage does not establish crash recovery, network-filesystem locking, concurrent rebuilds or SQL schema migrations. Freshness does not exhaust merge/rebase/shallow clone, ignored-file, non-UTF8 filename or concurrent-capture behavior; these controls do not establish model effectiveness.
- **Evaluator coverage remediation (FINDING-0005):** public contracts and TRX scoring are verified; four evaluators are strengthened/calibrated. Product-effectiveness claims remain disabled while stale propagation, deterministic code graph, change contracts, flight recorder, MCP validation and unfinished-verification policy remain. Next strengthen stale propagation without unrelated legacy shell prerequisites. See docs/evidence/benchmark-freshness-calibration-2026-09-03.md. Exposing compatibility requirements does not make partial assertions complete.
