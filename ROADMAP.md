...
- [x] Phase 66: bind token totals to captured host events, reject mismatched measurements, and preserve unavailable aggregate usage; verified by [CI run 33710861430](https://github.com/seekua/ArifCE/actions/runs/33710861430).
...
- [x] Phase 73: replace graph existence-only scoring with four pinned confidence, source-lifecycle, canonical-preserving cache-rebuild and trusted-closure tests. Unchanged product code, 101 tests on three OS targets, five binaries, independent integration and good/six-mutant calibration pass [CI run 33743344157](https://github.com/seekua/ArifCE/actions/runs/33743344157). See docs/evidence/benchmark-graph-calibration-2026-09-03.md.
- [x] Phase 74: strengthen change-contract linkage, risk/confidence, rejection-side-effect and scoped evidence/acceptance assertions with four pinned tests and good/six-mutant controls. Unchanged product code, 105 tests on three OS targets, five binaries and seven independent integrations pass [CI run 34105697809](https://github.com/seekua/ArifCE/actions/runs/34105697809). See docs/evidence/benchmark-contract-calibration-2026-09-03.md.
- [x] Phase 75: replace the flight-recorder existence test with three pinned security, lifecycle, failed-attempt and finite-growth evaluators. FINDING-0009 and FINDING-0010 are resolved; TASK-0025 is complete. The three-OS matrix, five self-contained packages, and Linux calibration controls passed in [CI run 34215866974](https://github.com/seekua/ArifCE/actions/runs/34215866974). See docs/evidence/benchmark-flight-recorder-calibration-2026-09-07.md.
- [x] Phase 76: replace MCP evaluator existence checks with canonical side-effect, malformed/oversized input, traversal, duplicate-write, and persisted-state assertions. FINDING-0011 and TASK-0026 are complete; remote controls passed in [CI run 34215866974](https://github.com/seekua/ArifCE/actions/runs/34215866974). See docs/evidence/benchmark-mcp-calibration-2026-09-07.md.
- [x] Phase 77: prevent unproven named commands and unsafe successful commands from becoming VERIFIED; verify persisted trust states and rejection-side effects. FINDING-0012 and TASK-0027 are complete; remote controls passed in [CI run 34215866974](https://github.com/seekua/ArifCE/actions/runs/34215866974). See docs/evidence/benchmark-verification-calibration-2026-09-08.md.
- [x] Phase 78: make the real model study repeatable and permission-matched: prepare two fresh sessions for each of ten task categories, require the same non-secret permission profile per pair, and reject product-study collection without captured token and host-time telemetry. The three-OS matrix, five self-contained packages, repeatability gate, provenance and all evaluator calibrations passed in [CI run 34285263692](https://github.com/seekua/ArifCE/actions/runs/34285263692). External model execution and effectiveness claims remain pending. See docs/evidence/benchmark-repeatability-gate-2026-09-09.md.
- [ ] Phase 79: make the external model study fair before resuming metered execution, then execute and publish all 20 matched pairs without favorable-result filtering. The balanced order gate passed [CI run 34295202034](https://github.com/seekua/ArifCE/actions/runs/34295202034); harness hardening and graph-contract corrections passed runs 34462957826 and 34466811725, while success-only treatment validation and failure-signature retry classification passed all eight jobs in [run 34889340578](https://github.com/seekua/ArifCE/actions/runs/34889340578). Four Terra/medium graph pairs remain diagnostic and ineligible; the fourth isolated a real model failure, so the graph task will not be repeated. The first predeclared `llm-secret-boundary` calibration attempt was also ineligible because both Windows structured edits failed before candidate creation. The harness now owns its Codex host arguments, pins the Windows restricted-token sandbox while ignoring user configuration, and rejects reparse/sandbox stderr as clean infrastructure. A real isolated structured-edit smoke run and all local regressions pass. The V1 `task-bounded-context` calibration has a fixed pre-feature fixture, public API contract, pinned two-test evaluator, passing known-good control and failing unchanged-fixture control; see [local calibration evidence](docs/evidence/core-lifecycle-calibration-2026-09-21.md). Its first matched Terra/medium pair produced evaluator-passing candidates in both arms but is [preserved as diagnostic and ineligible](docs/evidence/core-lifecycle-pair-diagnostic-2026-09-22.md): the treatment prompt named the wrong CLI assembly and the parser then failed to recognize the real workflow. A corrected clean-window pair is also [preserved as negative and ineligible](docs/evidence/core-lifecycle-corrected-pair-2026-09-22.md): the baseline failed the pinned foreign-task exclusion behavior, while the ArifCE candidate passed the evaluator but left its contracted task incomplete after overlapping test processes prevented evidence capture. No successful-task token comparison was made and the pair was not retried. The harness now serializes wrapper and evidence verification operations with a checkout-scoped mutex, disables MSBuild node reuse, and runs evidence tests without rebuilding; the complete cross-platform, packaging, benchmark, and evaluator matrix passed [CI run 35701316103](https://github.com/seekua/ArifCE/actions/runs/35701316103). A third [serialized pair](docs/evidence/core-lifecycle-serialized-pair-2026-09-22.md) produced evaluator-passing candidates in both arms and a fully eligible baseline, but the ArifCE workflow remained incomplete because the prompt prescribed invalid repeated `-Path` syntax and the retry did not finish. The wrapper now exposes one validated `-PathCsv` value and the contract requires one final evidence-producing suite with an explicit wait. A fourth [clean pair](docs/evidence/core-lifecycle-clean-pair-2026-09-22.md) preserved a fully eligible baseline and an ArifCE candidate that passed the external gates, but rejected the treatment because it repeated a failed `git commit` without an intervening re-plan. The result is immutable and makes no comparison claim. The neutral participant instructions now require an explicit re-plan before any materially similar retry, including a transient lock; the rejection policy itself is unchanged. A fifth [eligible pair](docs/evidence/core-lifecycle-eligible-pair-2026-09-24.md) passed every gate in both arms, but ArifCE consumed 63,523 more cache-excluded primary tokens and therefore establishes no savings claim. Lower-model trials are now permitted, but must use fresh predeclared matched arms. The 50,000 non-cached-input-plus-output target remains diagnostic. See docs/evidence/benchmark-harness-reliability-2026-09-10.md.

### Phase 79 addendum (2026-09-25)
...
- **Evaluator calibration versus effectiveness (FINDING-0005):** all ten evaluator objectives are now independently pinned, good/bad calibrated, and remotely executed. This closes the finite evaluator-remediation scope only. Product-effectiveness claims remain disabled until a fresh, repeated, permission-matched model study produces comparable host timing and provider usage evidence. Exposing compatibility requirements does not make model outcomes complete or favorable.
- **Active-work measurement and fair rerun:** Phase 67's monotonic process stopwatch excludes outer preparation/evaluation delay, but includes in-process network and approval waits. Active-work timing stays unavailable until these intervals are observable. Interrupted captures retain artifacts but require explicit failure reporting and a separately identified retry, not silent omission. Before the real matched rerun, verify equal permissions and prompt/evaluator contract alignment; the first run reported several API-shape mismatches whose fairness must be audited.
- **Benchmark measurement scope:** Phase 66 proves log-bound ingestion for single-turn Codex JSONL counters. Other host formats and multi-turn aggregation are deferred until their counter semantics can be tested without double counting. Active-time capture and the permission-controlled real benchmark rerun remain open; no new effectiveness result is claimed.
...
- **V0.8.1 release correction (not yet published):** current source and package-smoke coverage already implement task-aware context, task-focused handoff, and `task check`, but V0.8.0 predates those behaviors. The pending release candidate versions them as V0.8.1, exercises all three commands against each self-contained binary, and verifies executable mode metadata for Linux ZIPs. Local tests pass; native cross-platform release CI and publication are still required before this can be marked shipped.
