...
- [ ] Phase 4: checkpoints and Git snapshots work; dedicated decision/attempt authoring remains.
...
- [ ] Phase 7: deterministic command verification works; typed blind-review interfaces remain.
- [x] Phase 8: semantic handoffs from current state, task, checkpoint, claim, and Git state.
- [ ] Phase 9: campaign lifecycle, inventory blocking, and forbidden-reference guards work; authoring richer guard/inventory metadata through the CLI remains.
...
- [ ] Phase 13: build/tests and dogfood flow pass; license, cross-platform CI, package/install test, and the complete 16-step fixture remain release blockers.
...
- **Richer refactor authoring:** current campaign creation records title/objective and enforces persisted inventory/forbidden-reference guards. CLI options for invariants, workstreams, rollback points, and inventory mutation require a stable input contract.
...
- **Automated evidence kinds:** V0.1 records arbitrary deterministic command evidence. Structured build/test count parsers and API-diff adapters are not yet implemented.
