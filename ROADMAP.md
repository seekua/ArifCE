...
- **Phase 69 safety evaluator remediation:** two evaluators now check real Git/provider-call/persisted-secret boundaries and both successful and rejected high-risk acceptance. All 85 local behavior tests, both independent completion integrations and the known-good/seven-mutant calibration pass. Remote CI is pending. The other eight evaluator gaps, real multi-process/index coverage and new real A/B runs remain open; these synthetic controls do not establish model effectiveness.
- **Evaluator coverage remediation (FINDING-0005):** public contracts and TRX scoring are verified, but product-effectiveness claims remain disabled for these diagnostic scores. Before a comparative rerun, replace false-positive-prone secret/acceptance checks, add actual multi-process/index coverage, and calibrate all evaluators with correct/incorrect controls; see docs/evidence/benchmark-contract-audit-2026-09-03.md. Exposing compatibility requirements does not make partial assertions complete.
