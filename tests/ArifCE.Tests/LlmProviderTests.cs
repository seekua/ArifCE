...
public sealed class LlmProviderTests
{
    ...
    public async Task Task_context_pins_contract_and_never_leaks_it_past_budget()
    {
        ...
        try
        {
            ...
            await canonical.WriteAsync(root.FullName, "tasks", "TASK-0002", new TaskRecord(1, "TASK-0002", "Other task", null, WorkStatus.Open, RiskLevel.Low, DateTimeOffset.UtcNow));
            await canonical.WriteAsync(root.FullName, "attempts", "ATTEMPT-0001", new AttemptRecord(1, "ATTEMPT-0001", task.Id, "Use zebra cache", "failed", "Zebra cache repeats the race", [], DateTimeOffset.UtcNow));
            await canonical.WriteAsync(root.FullName, "attempts", "ATTEMPT-0002", new AttemptRecord(1, "ATTEMPT-0002", "TASK-0002", "Unrelated orchid payment approach", "failed", "Other task only", [], DateTimeOffset.UtcNow));
            await canonical.WriteAsync(root.FullName, "findings", "FINDING-0001", new FindingRecord(1, "FINDING-0001", "Quartz blocker", "Resolve quartz before completion", RiskLevel.High, WorkStatus.Open, task.Id, "Quartz.cs", DateTimeOffset.UtcNow));
            await canonical.WriteAsync(root.FullName, "claims", "CLAIM-0001", new ClaimRecord(1, "CLAIM-0001", "Amethyst behavior is preserved", ClaimStatus.Unverified, RiskLevel.Low, new GitSnapshot(null, "master", true, [], "fixture"), [], DateTimeOffset.UtcNow, task.Id));
            await canonical.WriteAsync(root.FullName, "claims", "CLAIM-0002", new ClaimRecord(1, "CLAIM-0002", "Unrelated payment behavior", ClaimStatus.Unverified, RiskLevel.Low, new GitSnapshot(null, "master", true, [], "fixture"), ["EVIDENCE-0002"], DateTimeOffset.UtcNow, "TASK-0002"));
            await canonical.WriteAsync(root.FullName, "evidence", "EVIDENCE-0002", new EvidenceRecord(1, "EVIDENCE-0002", "CLAIM-0002", "TEST_RUN", "dotnet test", 0, "Unrelated payment evidence", new GitSnapshot(null, "master", true, [], "fixture"), DateTimeOffset.UtcNow));
            await canonical.WriteAsync(root.FullName, "handoffs", "HANDOFF-0001", new HandoffRecord(1, "HANDOFF-0001", $"# Handoff: {task.Id}\n\nNext action: inspect cobalt path.\n", new GitSnapshot(null, "master", true, [], "fixture"), DateTimeOffset.UtcNow));
            await canonical.WriteAsync(root.FullName, "handoffs", "HANDOFF-0002", new HandoffRecord(1, "HANDOFF-0002", "# Handoff: TASK-0002\n\nUnrelated payment handoff.\n", new GitSnapshot(null, "master", true, [], "fixture"), DateTimeOffset.UtcNow));
            var composer = new LlmContextComposer(new IndexStore());
            var context = await composer.ComposeForTaskAsync(root.FullName, task.Id, 2000);
            Assert.Contains("Protect payment calculations", context.Content);
            Assert.Contains("Zebra cache repeats the race", context.Content);
            ...
            Assert.DoesNotContain("Unrelated orchid payment approach", context.Content);
            Assert.DoesNotContain("Unrelated payment evidence", context.Content);
            Assert.DoesNotContain("Unrelated payment handoff", context.Content);
            Assert.Contains(context.Items, item => item.Kind == "ATTEMPT" && item.Included && item.Reason.Contains("task-linked", StringComparison.Ordinal));
            var foreign = Assert.Single(context.Items, item => item.Path.EndsWith("attempt-0002.json", StringComparison.Ordinal));
            Assert.False(foreign.Included);
            Assert.Equal("OUT_OF_SCOPE", foreign.Freshness);
            Assert.Empty(foreign.Snippet);
